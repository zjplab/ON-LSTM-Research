...
def test(model, corpus, cuda, weight, figure, prt=False):
    ...
    for sen, sen_tree, sen_nltktree in dataset:
        ...
        gates = np.einsum('abc,ab->ac',combined_dis, weight)
        gates = np.sum(gates, axis=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wsj10', action='store_true',
                        help='use WSJ10')
    parser.add_argument('--gate', type=str, default="[[0,1,0],[0,0,0]]",
                        help="weights for each distance")   
    parser.add_argument('--figure',type=str, default="default",
                        help="Prefix for figure")
